...
class Player:
    ...
    ACTIONS = [FOLD, CALL, BET_1, BET_2, BET_3, BET_4, BET_5,
               BET_6, RAISE_1, RAISE_2, CHECK]
    ACTIONS_BET = [BET_1, BET_2, BET_3, BET_4, BET_5, BET_6]
    ACTIONS_RAISE = [RAISE_1, RAISE_2]

    bet_arr = [1/3, 1/2, 2/3, 1, 1.5, 2]

    bet_dict = {BET_1: 1/3, BET_2: 1/2, BET_3: 2/3,
                BET_4: 1, BET_5: 1.5, BET_6: 2}

    raise_arr = [2/3, 2]

    raise_dict = {RAISE_1: 2/3, RAISE_2: 2}
    ...
    BET_SIZE = [1/3, 1/2, 3/4, 1, 1.5, 2]
    starting_bet_distribution = [1/len(BET_SIZE)] * len(BET_SIZE)
    ...
    RAISE_SIZE = [2/3, 2]
    starting_raise_distribution = [1/len(RAISE_SIZE)] * len(RAISE_SIZE)
    ...
    def take_action(self, board, bets):
        ...
        def determine_action(dist, b, stage, stren):
            ...
            if b:
                if r >= Player.BET_1 and r <= self.BET_6 or r == self.CHECK:
                    ...
                    return (self.RAISE_1, self.raise_dict[self.RAISE_1])
                elif r == self.RAISE_2:
                    return (self.RAISE_2, self.raise_dict[self.RAISE_2])
            else:
                if r >= self.RAISE_1 and r <= self.RAISE_4 or r == self.FOLD or r == self.CALL:
                    ...
                    return (self.BET_1, self.bet_dict[self.BET_1])
                elif r == self.BET_2:
                    return (self.BET_2, self.bet_dict[self.BET_2])
                elif r == self.BET_3:
                    return (self.BET_3, self.bet_dict[self.BET_3])
                elif r == self.BET_4:
                    return (self.BET_4, self.bet_dict[self.BET_4])
                elif r == self.BET_5:
                    return (self.BET_5, self.bet_dict[self.BET_5])
                elif r == self.BET_6:
                    return (self.BET_6, self.bet_dict[self.BET_6])

        if len(board) == 0:
            strength = strength.starting_eval(
                Evaluator.prime_product_rank(self.cards))
            ...
            strength = strength.rank_breakdown(
                Evaluator.evaluate_rank(self.cards, board))
